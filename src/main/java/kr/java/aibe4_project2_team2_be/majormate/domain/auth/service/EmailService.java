...
public class EmailService {
	...
	public void verifyCode(String email, String code, VerificationType type) {
		EmailVerification verification = emailVerificationRepository
			...
			.orElseThrow(() -> new BadRequestException(ErrorCode.AUTH_400_VERIFICATION_CODE_NOT_FOUND));

		// 만료 확인
		if (verification.isExpired()) {
			throw new BadRequestException(ErrorCode.AUTH_400_VERIFICATION_CODE_EXPIRED);
		}

		// 이미 인증됨
		if (verification.isVerified()) {
			throw new BadRequestException(ErrorCode.AUTH_400_ALREADY_VERIFIED);
		}

		// 코드 일치 확인
		if (!verification.getCode().equals(code)) {
			throw new BadRequestException(ErrorCode.AUTH_400_INVALID_VERIFICATION_CODE);
		}
	...
	public void sendEmail(String to, String code, VerificationType type) {
		try {
			...
			throw new BadRequestException(ErrorCode.AUTH_500_EMAIL_SEND_FAILED);
		}
	}
