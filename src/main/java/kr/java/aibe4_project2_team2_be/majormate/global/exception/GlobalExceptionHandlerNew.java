...
public class GlobalExceptionHandlerNew {
	...
	public ResponseEntity<ApiResponseNew<Void>> handleBusinessException(BusinessExceptionNew e) {
		ErrorCodeNew errorCode = e.getErrorCode();
	...
	public ResponseEntity<ApiResponseNew<Void>> handleAuthenticationException(AuthenticationException e) {
		...
		return ResponseEntity
			.status(ErrorCodeNew.AUTH_401.getHttpStatus())
			.body(ApiResponseNew.error(ErrorResponseNew.of(ErrorCodeNew.AUTH_401)));
	}
	...
	public ResponseEntity<ApiResponseNew<Void>> handleAccessDeniedException(AccessDeniedException e) {
		...
		return ResponseEntity
			.status(ErrorCodeNew.AUTH_403.getHttpStatus())
			.body(ApiResponseNew.error(ErrorResponseNew.of(ErrorCodeNew.AUTH_403)));
	}
	...
	public ResponseEntity<ApiResponseNew<Void>> handleException(Exception e) {
		...
		return ResponseEntity
			.status(ErrorCodeNew.COMMON_500.getHttpStatus())
			.body(ApiResponseNew.error(ErrorResponseNew.of(ErrorCodeNew.COMMON_500)));
	}

	private ResponseEntity<ApiResponseNew<Void>> badRequest() {
		return ResponseEntity
			.status(HttpStatus.BAD_REQUEST)
			.body(ApiResponseNew.error(ErrorResponseNew.of(ErrorCodeNew.COMMON_400)));
	}

	private ResponseEntity<ApiResponseNew<Void>> badRequestWithDetails(Object details) {
		return ResponseEntity
			.status(HttpStatus.BAD_REQUEST)
			.body(ApiResponseNew.error(ErrorResponseNew.of(ErrorCodeNew.COMMON_400, details)));
	}
